...
func request(conn net.Conn) string {
	...
	for scanner.Scan() {
		ln := scanner.Text()
		if i == 0 {
			method := strings.Fields(ln)[1]
			fmt.Println("METHOD : ", method)
		} else if i == 1 {
			url = strings.Fields(ln)[1]
		} else {
			fmt.Println("content received was : ", ln)
			//fmt.Fprintf(conn, "You sent this: %s", ln)
		}
		if ln == "" {
			// There is a blank line before the http body
			break
		}
		i++
